//  
//  LiveCreateCoverReusableView.swift
//  MALL_iOS
//
...
extension LiveCreateCoverReusableView: LiveCreateUpdateTitle {
    public func updateTitle(_ title: String) {
        cardView.titleView.updateTitle(title)
    }
}

class LiveCreateCoverReusableView: UICollectionReusableView {
    // MARK: - Properties
    /// 直播封面url
    public private(set) var studioLogo: String?
    ...
    public private(set) lazy var coverButton: UIButton = {
        ...
        button.setBackgroundImage(UIImage(named: "live_cover"), for: .normal)
        ...
        button.addTarget(self, action: #selector(coverButtonClick), for: .touchUpInside)
        return button
    }()
    ...
    private lazy var imageView: UIImageView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(coverButtonClick))
...
private extension LiveCreateCoverReusableView {
    func setupUI() {
        ...
        [coverButton, imageView].forEach { bottomView.addSubview($0) }
        coverButton.snp.makeConstraints { (make) in
            make.top.equalTo(bottomView.snp.top)
            make.left.equalToSuperview()
            make.width.equalTo(74.auto())
